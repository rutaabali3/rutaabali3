...
def card(x: int, color: str, label: str, value: int, detail: str) -> str:
    return f'''  <rect x="{x}" y="82" width="204" height="154" rx="12" fill="#161b22" stroke="#30363d"/>
  <circle cx="{x + 28}" cy="116" r="12" fill="{color}" opacity=".22"/>
  <circle cx="{x + 28}" cy="116" r="5" fill="{color}"/>
  <text x="{x + 52}" y="121" fill="#c9d1d9" font-family="Segoe UI,Arial,sans-serif" font-size="13" font-weight="600">{label}</text>
  <text x="{x + 24}" y="174" fill="#f0f6fc" font-family="Segoe UI,Arial,sans-serif" font-size="34" font-weight="700">{value}</text>
  <text x="{x + 24}" y="202" fill="#8b949e" font-family="Segoe UI,Arial,sans-serif" font-size="12">{detail}</text>'''
...
def main() -> None:
  ...
  <defs>
    <linearGradient id="bg" x1="0" x2="1" y1="0" y2="1"><stop stop-color="#0d1117"/><stop offset="1" stop-color="#1f2633"/></linearGradient>
  </defs>
  <rect width="900" height="270" rx="16" fill="url(#bg)" stroke="#30363d"/>
  <text x="30" y="34" fill="#f0f6fc" font-family="Segoe UI,Arial,sans-serif" font-size="20" font-weight="700">Commit activity</text>
  <text x="30" y="58" fill="#8b949e" font-family="Segoe UI,Arial,sans-serif" font-size="12">Live metrics from GitHub · refreshed daily</text>
{card(24, '#58a6ff', 'Last 24 hours', values['last_24h'], 'commits')}
{card(240, '#56d364', 'Current streak', values['current_streak'], 'consecutive days')}
{card(456, '#a371f7', 'Most in one day', values['most_in_day'], 'commits on best day')}
{card(672, '#f0883e', 'Longest streak', values['longest_streak'], 'consecutive days')}
</svg>'''
